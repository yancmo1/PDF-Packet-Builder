# PR Fix (Implementation) — PR #{{PR_NUMBER}} — {{SHORT_TOPIC}}
...
- Review artifact (most recent if present): docs/reviews/{{REVIEW_FILENAME}}
- Reply scratchpad (optional): docs/reviews/{{REPLY_FILENAME}}
...
## What to do
1) Read `docs/reviews/{{REVIEW_FILENAME}}` if it exists (or the review content I paste in chat) and extract:
